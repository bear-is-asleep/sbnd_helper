...
class ParticleGroup(CAF):
    ...
    def add_stat_unc(self, nuniv=100, progress_bar=False, seed=69):
        ...
        Each event gets a deterministic seed derived from its index tuple,
        so the Poisson weight for a given (event, universe) pair is stable
        regardless of dataset ordering or filtering.
        ...
        progress_bar : bool
            Show progress bars while generating universes
        seed : int or None
            Base seed mixed into every per-event hash for full reproducibility.
        ...
        seed_bytes = str(seed).encode() if seed is not None else b''

        # Deterministic per-event seed from sha256(base_seed + index_tuple)
        event_seeds = np.empty(n_events, dtype=np.uint32)
        for i in tqdm(range(n_events), desc="Generating event seeds", disable=not progress_bar):
            h = hashlib.sha256(seed_bytes + str(self.data.index[i]).encode()).digest()
            event_seeds[i] = int.from_bytes(h[:4], 'little')

        all_poisson_weights = []
        for uidx in tqdm(range(nuniv), desc="Generating universes", disable=not progress_bar):
            univ_h = hashlib.sha256(seed_bytes + f"univ_{uidx}".encode()).digest()
            univ_seed = int.from_bytes(univ_h[:4], 'little')
            combined = (event_seeds.astype(np.uint64) + np.uint64(univ_seed)) % np.uint64(2**32)

            poisson_weights = np.array(
                [np.random.default_rng(int(c)).poisson(1.0) for c in combined],
                dtype=float,
            )
            all_poisson_weights.append(poisson_weights)
